...
public final class NumberOutput {
    ...
    public static int outputInt(int value, char[] buffer, int offset) {
        ...
        if (value < MILLION) {
            if (value >= DefaultDrawerConf.QUILL_HEIGHT_UM) {
                int thousands = value / DefaultDrawerConf.QUILL_HEIGHT_UM;
                offset = outputFullTriplet(value - (thousands * DefaultDrawerConf.QUILL_HEIGHT_UM), buffer, outputLeadingTriplet(thousands, buffer, offset));
            } else if (value < 10) {
                offset2 = offset + 1;
        ...
        int newValue = value / DefaultDrawerConf.QUILL_HEIGHT_UM;
        int ones = value - (newValue * DefaultDrawerConf.QUILL_HEIGHT_UM);
        value = newValue;
        newValue /= DefaultDrawerConf.QUILL_HEIGHT_UM;
        thousands = value - (newValue * DefaultDrawerConf.QUILL_HEIGHT_UM);
        if (hasBillions) {
    ...
    public static int outputLong(long value, char[] buffer, int offset) {
        ...
        while (ivalue >= DefaultDrawerConf.QUILL_HEIGHT_UM) {
            ptr -= 3;
            int newValue2 = ivalue / DefaultDrawerConf.QUILL_HEIGHT_UM;
            outputFullTriplet(ivalue - (newValue2 * DefaultDrawerConf.QUILL_HEIGHT_UM), buffer, ptr);
            ivalue = newValue2;
        }
