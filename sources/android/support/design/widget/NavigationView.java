...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    public static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C00581());
        public Bundle menuState;

        /* renamed from: android.support.design.widget.NavigationView$SavedState$1 */
        static class C00581 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C00581() {
            }
    ...
    public NavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0369R.styleable.NavigationView, defStyleAttr, C0369R.style.Widget_Design_NavigationView);
        setBackgroundDrawable(a.getDrawable(C0369R.styleable.NavigationView_android_background));
        if (a.hasValue(C0369R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0369R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, a.getBoolean(C0369R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.mMaxWidth = a.getDimensionPixelSize(C0369R.styleable.NavigationView_android_maxWidth, 0);
        if (a.hasValue(C0369R.styleable.NavigationView_itemIconTint)) {
            itemIconTint = a.getColorStateList(C0369R.styleable.NavigationView_itemIconTint);
        } else {
            itemIconTint = createDefaultColorStateList(16842808);
        }
        boolean textAppearanceSet = false;
        int textAppearance = 0;
        if (a.hasValue(C0369R.styleable.NavigationView_itemTextAppearance)) {
            textAppearance = a.getResourceId(C0369R.styleable.NavigationView_itemTextAppearance, 0);
            textAppearanceSet = true;
        }
        ColorStateList itemTextColor = null;
        if (a.hasValue(C0369R.styleable.NavigationView_itemTextColor)) {
            itemTextColor = a.getColorStateList(C0369R.styleable.NavigationView_itemTextColor);
        }
        if (!textAppearanceSet && itemTextColor == null) {
            itemTextColor = createDefaultColorStateList(16842806);
        }
        Drawable itemBackground = a.getDrawable(C0369R.styleable.NavigationView_itemBackground);
        this.mMenu.setCallback(new C00571());
        ...
        if (a.hasValue(C0369R.styleable.NavigationView_menu)) {
            inflateMenu(a.getResourceId(C0369R.styleable.NavigationView_menu, 0));
        }
        if (a.hasValue(C0369R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(a.getResourceId(C0369R.styleable.NavigationView_headerLayout, 0));
        }
        a.recycle();
    }
    ...
    private ColorStateList createDefaultColorStateList(int baseColorThemeAttr) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0369R.attr.colorControlActivated, value, true)) {
            return null;
        }
        int colorControlActivated = value.data;
        int defaultColor = baseColor.getDefaultColor();
        return new ColorStateList(new int[][]{DISABLED_STATE_SET, CHECKED_STATE_SET, EMPTY_STATE_SET}, new int[]{baseColor.getColorForState(DISABLED_STATE_SET, defaultColor), colorControlActivated, defaultColor});
    }
}
