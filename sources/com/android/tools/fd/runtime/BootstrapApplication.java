...
public class BootstrapApplication extends Application {
    ...
    /* renamed from: com.android.tools.fd.runtime.BootstrapApplication$1 */
    static class C00011 implements Logging {
        C00011() {
        }
        ...
        public boolean isLoggable(Level level) {
            if (level == Level.SEVERE) {
                return Log.isLoggable(BootstrapApplication.LOG_TAG, 6);
            }
            if (level == Level.FINE) {
                return Log.isLoggable(BootstrapApplication.LOG_TAG, 2);
            }
            return Log.isLoggable(BootstrapApplication.LOG_TAG, 4);
        }
        ...
        public void log(Level level, String string, Throwable throwable) {
            ...
            } else if (!Log.isLoggable(BootstrapApplication.LOG_TAG, 4)) {
            } else {
                if (throwable == null) {
    ...
    public BootstrapApplication() {
        if (Log.isLoggable(LOG_TAG, 2)) {
            Log.v(LOG_TAG, String.format("BootstrapApplication created. Android package is %s, real application class is %s.", new Object[]{AppInfo.applicationId, AppInfo.applicationClass}));
        }
    }
    ...
    private void createResources(long apkModified) {
        ...
        if (Log.isLoggable(LOG_TAG, 2)) {
            Log.v(LOG_TAG, "Resource override is " + this.externalResourcePath);
        }
        if (file != null) {
            try {
                ...
                if (Log.isLoggable(LOG_TAG, 2)) {
                    ...
                    Log.v(LOG_TAG, "APK last modified: " + apkModified + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + (apkModified > resourceModified ? ">" : "<") + " resource patch");
                }
    ...
    private void createRealApplication() {
        if (AppInfo.applicationClass != null) {
            if (Log.isLoggable(LOG_TAG, 2)) {
                Log.v(LOG_TAG, "About to create real application of class name = " + AppInfo.applicationClass);
            }
            try {
                Class<? extends Application> realClass = Class.forName(AppInfo.applicationClass);
                if (Log.isLoggable(LOG_TAG, 2)) {
                    Log.v(LOG_TAG, "Created delegate app class successfully : " + realClass + " with class loader " + realClass.getClassLoader());
                }
                this.realApplication = (Application) realClass.getConstructor(new Class[0]).newInstance(new Object[0]);
                if (Log.isLoggable(LOG_TAG, 2)) {
                    Log.v(LOG_TAG, "Created real app instance successfully :" + this.realApplication);
                    return;
                }
                return;
            } catch (Exception e) {
            ...
            try {
                ...
                if (processes == null || processes.size() <= 1) {
                    ...
                    if (!(startServer || foundPackage)) {
                        startServer = true;
                        if (Log.isLoggable(LOG_TAG, 2)) {
                            Log.v(LOG_TAG, "Multiprocess but didn't find process with package: starting server anyway");
                        }
                    }
                }
                if (startServer) {
